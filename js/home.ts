...
import { initSplashScreen, resetSplashSession } from './splashScreen';
...
document.addEventListener('DOMContentLoaded', () => {
    ...
    // Replay Splash Screen button handler
    const replaySplashBtn = document.getElementById('replay-splash-btn');
    if (replaySplashBtn) {
        replaySplashBtn.addEventListener('click', (e) => {
            e.preventDefault();
            resetSplashSession();
            initSplashScreen({ forceShow: true, duration: 2500 });
        });
    }
